...
public class hormigas_17_red_brain_9 extends PApplet {
...
public void setup() {
  ...
  for (int i = 0; i < numboids; i++) {
    boids.add(new Boid(new PVector(random(width), random(height)), random(0.1f, 0.5f)));
    boids3.add(new Boid3(new PVector(random(width), random(height)), random(0.1f, 0.5f)));
  }
  for (int j = 0; j < numboids*2; j++) {
    boids2.add(new Boid2(new PVector(random(width), random(height)), random(0.1f, 0.5f)));
    boids4.add(new Boid4(new PVector(random(width), random(height)), random(0.1f, 0.5f)));
  }
  //graphic setup
  background(0);
  ellipseMode(CENTER);
...
public void serialEvent(Serial myPort) {
  ...
  flexValuesInt = PApplet.parseFloat(split(flexSerial, ','));
...
public void userInterface(PVector dot) {
  ...
  if (human) {
    ...
    text("Position "+PApplet.parseInt(x)+" "+PApplet.parseInt(z), 10, 115);
  }
  else {
    fill(255, 0, 0);
...
class FlowField {
  ...
  public int[] lookupIndex(PVector lookup) {
    ...
    int row = PApplet.parseInt(constrain(z/resolution, 0, cols-1));
    int[] r = { 
      column, row
    };
    return r;
  ...
  public PVector[][] lookuparea(PVector lookup, int s) {
...
class Boid2 {
  ...
  public void separate (ArrayList<Boid2> boids2) {
...
class Boid3 {
  ...
  public void applyBehaviors(ArrayList<Boid3> boids3, PVector dot) {
  ...
  public PVector separate (ArrayList<Boid3> boids3) {
    float desiredseparation = r;
    PVector sum = new PVector();
    int count = 0;
    // For every boid in the system, check if it's too close
...
class Boid4 {
  ...
  public void separate (ArrayList<Boid4> boids4) {
    float desiredseparation = r*2;
    PVector sum = new PVector();
    int count = 0;
    // For every boid in the system, check if it's too close
...
class mouseLocation {
  ...
  public void display(PVector dot) {
    float z = map(dot.z, 0, 1500, -300, height-500);
    float x = map(dot.x, -500, 500, 0, width+100);
    println(dot.x);
    fill(0);
  ...
  public void mouseblock( FlowField flow, PVector dot ) {
  ...
  public void mousel(FlowField flow, PVector lol) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "hormigas_17_red_brain_9" };
    if (passedArgs != null) {
